...
class SizeFinder:
    ...
    def findSize(self, imageData):
        ...
        for i, fishID in enumerate(blobsData):
            ...
            originalImage = cv2.rectangle(imageData.img,[mininumPointX[0],mininumPointY[1]], [maximumPointX[0],maximumPointY[1]], colours[i], 2)

            cv2.drawContours(imagePlot, [sortedContours[i]], -1, colours[i], -1)  # , colours[i], thickness=cv2.FILLED)

            lineColor = self.singleRGBcolor(round(255 / 2))
