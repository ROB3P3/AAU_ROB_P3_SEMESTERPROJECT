...
def taskHandeler(indexFileNameList, startingNumber, group, outputDataRootPath, TH, sizeFinder, grippingPoints,
    ...
    for i, fileName in enumerate(indexFileNameList):
        ...
        if i not in ignoreList:
            ...
            csvHeader = ["group id", "image id", "fish index", "species", "length", "width", "area", "gripping points", "center point", "orientations", "avg hsv", "true negatives"]
...
def logicHandle(pathInputRoot, groups):
    ...
    clf = Classifier()
    gaussianClassifer = clf.createClassifier("DATAdir/annotated_training_data.csv", "DATAdir/annotated_validation_data.csv")
